...
export async function GET(request: Request) {
  try {
    ...
    return ok({ items: await distinctStrings("SELECT DISTINCT trim AS value FROM vehicle_paths WHERE year=? AND make=? AND model=? ORDER BY trim COLLATE NOCASE", [year, make, model]) });
